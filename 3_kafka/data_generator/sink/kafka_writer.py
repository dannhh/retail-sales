...
class KafkaWriterConfiguration:
    """
    __init__(): Initialize broker, topics, producer
    create_or_recreate_topics(): 
        - check all topics need to be created / recreated
        - set admin if not set
        - delete topics already exists (in this case you want to recreate it)
        - create topics with 5 retries
    send_message():
        - set producer if not set
        - produce key, value into topic queue >> if buffer error, flush and reproduce
    """
